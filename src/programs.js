...
function MediaPlayer(file_path) {
	// TODO: DRY the default file names and title code (use document.title of the page in the iframe, in make_iframe_window)
	var document_title = file_path ? file_name_from_path(file_path) : "Media";
	var win_title = document_title + " - Win95 Media Player";
	// TODO: focus existing window if file is currently open?
	var $win = make_iframe_window({
		src: "programs/media-player/example/quickstart/quickstart.html" + (file_path ? ("?path=" + file_path) : ""),
		icons: iconsAtTwoSizes("media-player"),
		title: win_title,
		innerWidth: 360,
		innerHeight: 340 + 21,
		minInnerWidth: 270,
...
function Spider() {
	var $win = make_iframe_window({
		src: "programs/spider/spider.html",
...
function Maze() {
	const options = { hideUI: true };
	showScreensaver("programs/maze/index.html");
}
